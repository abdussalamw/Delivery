...
app.get('/status', (req, res) => {
    res.json({ connected: isConnected, phone: connectedPhone, hasQR: !!currentQR, uptime: process.uptime() });
});
...
app.post('/wa/pair', async (req, res) => {
    ...
    const check = () => {
        ...
        if (waited > 15000) return res.json({ error: 'Timeout' });
        waited += 500;
...
app.post('/send', async (req, res) => {
    ...
    try {
        await sock.sendMessage(phone.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text: message });
        res.json({ ok: true });
    } catch (e) {
